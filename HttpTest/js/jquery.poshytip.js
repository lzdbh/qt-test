...
(function($) {
	...
	$.Poshytip.prototype = {
			...
			if (this.opts.showOn != 'none') {
				...
				switch (this.opts.showOn) {
					case 'hover':
						...
						break;
					case 'click':						
						this.$elm.bind('click.poshytip', $.proxy(this.showDelayed, this));
						if (this.opts.allowTipHover)
							this.$tip.hover($.proxy(this.clearTimeouts, this), $.proxy(this.mouseleave, this));
						break;
					case 'focus':
		...
		mouseenter: function(e) {
			...
			this.$elm.attr('title', '');
			if (this.opts.showOn == 'focus'||this.opts.showOn == 'click')
				return true;
		...
		if (opts.liveEvents && opts.showOn != 'none') {
			...
			switch (opts.showOn) {
				...
				case 'click':
					...
					// support 1.4.2+ & 1.9+
					this.live ?
						this.live('click.poshytip', handler) :
						$(document).delegate(this.selector, 'click.poshytip', handler);
					break;
				case 'focus':
					handler = function() {
						var $this = $(this);
